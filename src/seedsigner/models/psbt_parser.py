...
class PSBTParser():
    ...
    def has_matching_input_fingerprint(
        ...
        *,
        ...
        """Extracts fingerprint from each psbt input. Returns True if any match."""
        if seed is not None:
        ...
        for inp in psbt.inputs:
            for pub, derivation_path in inp.bip32_derivations.items():
                if seed_fingerprint == hexlify(derivation_path.fingerprint).decode():
                    return True
            for pub, (leaf_hashes, derivation_path) in inp.taproot_bip32_derivations.items():
                if seed_fingerprint == hexlify(derivation_path.fingerprint).decode():
                    return True
        return False
    ...
    def _fill_missing_fingerprints(self):
        ...
        # Only HDKey supports .child() and .derive(); WIFKey stores a raw PrivateKey
        if not hasattr(self.root, "child"):
            return 0
