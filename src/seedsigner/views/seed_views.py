        ...
        if len(script_types) == 1:
            ...
            if (
                ...
                if self.controller.resume_main_flow == Controller.FLOW__ADDRESS_EXPLORER:
                    ...
                    return Destination(AccountNumberView, view_args=dict(next_view_cls=SeedExportXpubQRFormatView, next_view_args=args), skip_current_view=True)
            else:
                if self.controller.resume_main_flow == Controller.FLOW__ADDRESS_EXPLORER:
                    del args["sig_type"]
        ...
        if selected_menu_num == RET_CODE__BACK_BUTTON:
            ...
            if (
                ...
                if self.controller.resume_main_flow == Controller.FLOW__ADDRESS_EXPLORER:
                    ...
                    return Destination(AccountNumberView, view_args=dict(next_view_cls=SeedExportXpubQRFormatView, next_view_args=args))
            else:
                if self.controller.resume_main_flow == Controller.FLOW__ADDRESS_EXPLORER:
                    del args["sig_type"]
...
class SeedExportXpubVerifyAddressView(View):
    ...
    def run(self):
        ...
        self.run_screen(
            ...
            status_headline=None,
            text=_("Open {} and display a receive address from the wallet you just exported.").format(self.coordinator_label or _("your wallet software")),
...
class SeedWordsBackupTestView(View):
    ...
    def run(self):
        ...
        fake_word1 = ButtonOptionWithoutTranslation("".join(bip39.WORDLIST[int(random.random() * 2047)]))
        fake_word2 = ButtonOptionWithoutTranslation("".join(bip39.WORDLIST[int(random.random() * 2047)]))
        fake_word3 = ButtonOptionWithoutTranslation("".join(bip39.WORDLIST[int(random.random() * 2047)]))
...
class MultisigWalletDescriptorView(View):
    ...
    def run(self):
        ...
        if descriptor.is_basic_multisig:
            ...
            # Single-sig descriptors (e.g. Satochip xpub-only loading) are also
            # routed through this view; fall back to embit's brief policy text.
