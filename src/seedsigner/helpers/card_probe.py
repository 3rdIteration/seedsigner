...
def count_keycard_instances(controller, timeout_s: float = 1.5) -> int:
    """Count the Keycard applet instances installed on the inserted card.

    Uses cleartext ``SELECT AID`` probes (no ISD keys, no PIN) over the
    **full** instance-AID range SeedSigner allocates
    (``A000000804000101`` + ``0x01`` + ``0x01..MAX``) plus the currently
    active AID — so a keycard-shell card on a non-default suffix still
    counts as one. Each full 10-byte AID exact-matches a single instance
    (unlike the 9-byte prefix probes used elsewhere for "find any"), so
    the number of ``SW=0x9000`` responses is the instance count.

    Best-effort: never raises; returns ``0`` when no reader/card is
    present or the smartcard stack is unavailable. The top Keycard menu
    treats any value != 1 as "keep Switch instance visible", so an
    under-count can never hide the only way to switch.
    """
    try:
        from seedsigner.helpers.keycard.client import KeycardClient
        from seedsigner.helpers.keycard.commands import APDUError
        ...
        from seedsigner.helpers.keycard.ui_helpers import _KEYCARD_INSTANCE_PREFIX
    except ImportError as exc:
        logger.debug("smartcard stack unavailable: %s", exc)
        return 0
    ...
    # Full 10-byte instance AIDs: prefix + 0x01 (group byte) + index.
    candidates: list[bytes] = []
    active = getattr(controller, "active_keycard_aid", None)
    if active:
        candidates.append(bytes(active))
    for index in range(0x01, 0x01 + MAX_KEYCARD_INSTANCES):
        aid = _KEYCARD_INSTANCE_PREFIX + bytes([0x01, index])
    ...
    try:
        ...
        logger.debug("count_keycard_instances failed: %s", exc)
        return 0
    finally:
        if connection is not None:
            try:
