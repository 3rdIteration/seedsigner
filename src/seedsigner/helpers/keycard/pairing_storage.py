...
def encrypt_pairing(
    ...
    # Instance naming lives in a separate microSD JSON file now (see
    # ``instance_names``); the trailer is still emitted (empty) so the on-disk
    # blob stays byte-compatible with older firmware and the read path keeps
    # decoding any legacy name without error.
    payload = (
