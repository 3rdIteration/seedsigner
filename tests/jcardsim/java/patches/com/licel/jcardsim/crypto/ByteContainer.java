...
/**
 * PATCHED COPY of jcardsim 3.0.5's com.licel.jcardsim.crypto.ByteContainer.
 *
 * Compiled at test time and placed ahead of the jcardsim jar on the simulator's
 * classpath by _patched_classes() in tests/jcardsim/simulator.py. Only the allocation in
 * setBytes() differs from upstream:
 *
 *   upstream: allocate once, on first use, sized to that first value and never again.
 *   patched:  (re)allocate whenever the incoming value's length differs from the buffer.
 *
 * Why: setBigInteger() stores BigInteger.toByteArray() with a leading zero trimmed, so
 * the same container is asked for 31 bytes or 32 bytes depending on the random value.
 * Upstream copies the longer value into the shorter buffer and dies with
 * ArrayIndexOutOfBoundsException -- e.g. Keycard's SecureChannel.initSecureChannel()
 * generating a secp256k1 key pair, where roughly one random scalar in 256 (top byte
 * zero) was a 31-byte value. The simulator runtime catches that Throwable and returns
 * SW=6F00, so the applet silently failed to initialise instead of erroring usefully.
 *
 * Reallocating on any length change also keeps the data.length == length invariant,
 * which is what getBigInteger() (new BigInteger(1, data)) relies on; growing alone would
 * leave a stale trailing byte behind when a longer value is followed by a shorter one.
 *
