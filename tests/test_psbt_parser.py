...
def raises_reject(code):
    """
    The fork folds upstream's PSBTVerificationError subclasses into one
    InvalidPSBTError carrying a RejectCode, so an upstream `pytest.raises(SomeError)`
    becomes a raises() plus a check on the code. See psbt_parser.RejectCode.
    """
    import pytest
    class _Ctx:
        def __enter__(self):
            self._raises = pytest.raises(InvalidPSBTError)
...
class TestPSBTParser:
    ...
    def test_missing_fingerprint_handling(self):
        ...
        # Rewriting witness_utxo leaves any non_witness_utxo describing the original
        # script, and _validate_input refuses a prevout described two inconsistent ways
        # (the BIP-143 amount-binding shape). A segwit input carrying witness_utxo alone
        # is legal and is what this fixture means.
    ...
    assert pp.change_data == [
        {
            ...
            'amount': 2871443918,
            'verified_derivation_path': [2147483734, 2147483649, 2147483648, 1, 1]}
        ]
    ...
    assert psbt_parser.change_data == [
        {
            ...
            'amount': 99992296,
            'verified_derivation_path': [2147483732, 2147483649, 2147483648, 0, 2]}
        ]
...
class TestPSBTParserSeedOwnership:
    ...
    def test__parse__verified_derivation_paths_none_for_not_owned_input(self):
        ...
        # still describes our own script, which _validate_input reads as a prevout
        # described two ways. A real payjoin input carries witness_utxo alone anyway.
    ...
    def test__parse__rejects_a_forged_claim_on_an_input(self):
        """
        `parse` should raise RejectCode.FORGED_INPUT_OWNERSHIP when a psbt carries a false
        ...
        with raises_reject(RejectCode.FORGED_INPUT_OWNERSHIP):
            self._parse(psbt)


    def test__parse__rejects_a_forged_claim_on_an_output(self):
        """
        `parse` should raise RejectCode.FORGED_OUTPUT_OWNERSHIP when a psbt carries a false
        ...
        with raises_reject(RejectCode.FORGED_OUTPUT_OWNERSHIP):
            self._parse(psbt)
    ...
    def test__parse__rejects_a_seed_that_owns_no_input(self):
        ...
        RejectCode.SEED_CANNOT_SIGN rather than letting the flow discover it after the user
        has approved.
    ...
    def test_a_psbt_with_no_utxos_is_rejected_rather_than_crashing(self):
        ...
        RejectCode.SEED_CANNOT_SIGN. Note that diagnosing the malformation is not this check's
        job.
        """
        psbt = self._psbt_with_change()
...
class TestPSBTParserOutputOwnership(TestPSBTParserSeedOwnership):
    """
    ...
    def test__parse__rejects_a_single_key_output_that_claims_more_than_one_path(self):
        ...
        with raises_reject(RejectCode.SURPLUS_DERIVATIONS):
            self._parse(psbt)
    ...
    def test__parse__rejects_an_output_with_entries_in_both_derivation_path_maps(self):
        ...
        with raises_reject(RejectCode.MIXED_DERIVATION_MAPS):
            self._parse(psbt)
    ...
    def test__parse__rejects_an_output_that_pays_this_seed_but_claims_someone_else(self):
        ...
        with raises_reject(RejectCode.MISLABELED_OUTPUT_OWNERSHIP):
            self._parse(psbt)
    ...
    def test__parse__refuses_a_multisig_decoy_entry_in_either_position(self):
        ...
        for input_base64, change_hex in [
            ...
            for decoy_first in [True, False]:
                ...
                # Both orderings are refused by the fork's ownership scan, which runs
                # before the change classification: a genuine key of ours annotated on
                # an output whose script does not pay it is a forged ownership claim,
                # regardless of where the entry sits. (Upstream reported the decoy-last
                # ordering as a surplus derivation instead, since it classified outputs
                # before checking each claim against the committed script.)
                expected_code = RejectCode.FORGED_OUTPUT_OWNERSHIP

                with raises_reject(expected_code):
                    self._parse(psbt)
    ...
    def test__parse__counts_a_different_quorum_as_an_unidentified_output_if_no_global_xpubs(self):
        """
        ...
        The global xpubs are needed in order to resolve cosigners. Without them, nothing
        ties the output's other keys to the wallet the inputs spend from: the user's seed
        IS in the output script and the output's policy "shape" matches the input's
        (2-of-3, same script type), but a different 2-of-3 that shares our key is
        indistinguishable from the wallet's own change.

        The fork therefore does not presume change here. The output is counted as a
        payment and recorded in `unidentified_change_outputs`, which is what drives
        PSBTIdentifyChangeView: the user can load the wallet's descriptor to identify it
        as change. Upstream instead presumed change whenever the cosigner comparison
        could not run.
        ...
        for input_base64, change_hex in [
            ...
            # Without a descriptor to identify it, the output is a payment awaiting
            # identification rather than presumed change.
            assert psbt_parser.change_amount == 0
            assert psbt_parser.spend_amount == 10_000
            assert psbt_parser.unidentified_change_outputs == [0]


    def test__parse__refuses_an_unsupported_script_type(self):
        """
        Parsing should be aborted if a psbt carries a script type that embit doesn't
        recognize. embit reports unhandled types as None.

        The fork validates inputs before it classifies outputs, so a fabricated
        witness_utxo describing a p2pk prevout is refused at that earlier stage
        (INVALID_WITNESS_UTXO) rather than reaching the output-side policy catch-all.
        This covers the output side instead: a valid input whose output pays an
        unrecognized script has no address representation, so it cannot be authorized
        and is refused as UNDISPLAYABLE_OUTPUT. The catch-all in _parse_outputs remains
        a defensive safety net for a matching pair of unrecognized types.
        """
        root = self._root()
        ...
        psbt = self._psbt_with_change()
        # Leave the input valid; only the output uses the unrecognized script type.
        ...
        with raises_reject(RejectCode.UNDISPLAYABLE_OUTPUT):
            self._parse(psbt)
