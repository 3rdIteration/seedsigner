...
* GPG Tools
   - GPG Signature verification & Sha256 Manifest check (Includes pubkey bundle the from Sparrow to verify Seedsigner, Sparrow, Electrum, plus many more)
    ...
    - Advanced submenu exposes Subkey Operations to add, revoke, delete, change expiry, or export subkey secrets (file, QR, or smartcard) with passphrase protection; BIP85-derived keys use deterministic BIP85 subkeys with successive indexes and only the latest subkey may be deleted
