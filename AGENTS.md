...
**Joined-plaintext-string wipes (a separate concern from the wordlist-copy rule above).** The seed import / generate / Seedkeeper-backup flows build a *joined* mnemonic string (`" ".join(words)`) and a *decoded* passphrase string (`passphrase_buf.decode()`) — each an immutable allocation holding the **whole** secret in one object, which the per-word `wipe_list()` does **not** reach. Every such flow now also routes those two locals through `wipe_string()` in a `finally` on **every** exit path (success, user-cancel, and error): `ToolsKeycardImportSeedView`, `ToolsKeycardImportSeedkeeperPushView`, `ToolsKeycardGenerateSeedLoadView`, and `ToolsKeycardSeedkeeperSaveRunView` (which gained a `try/finally` for this; its `secret_list` / `mnem_bytes` / `pp_bytes` int-lists can't be wiped — CPython small-int interning — so they are overwrite-then-`.clear()`ed best-effort). The wipe is best-effort (CPython GC) and **never** touches the controller's `pending_*` copies, which stay under the error-vs-user-exit rule (so the Seedkeeper "Both" flow keeps the seed for the second save). Any **new** flow that joins/decodes a mnemonic or passphrase into a `str` must wipe it the same way. Cover: `tests/test_keycard_secret_wipe.py`.
...
**Swap-screen redirect exemption.** A card removal normally snaps the UI back to Home (the centralised `Controller._on_card_removed_redirect`, *separate* from the secret wipe — see "Threat model and memory hygiene"). On the three screens that **instruct** the user to pull the card mid-flow — `ToolsKeycardSeedkeeperSwapInsertView` (backup "Both"/"Another card") and the import-from-Seedkeeper `ToolsKeycardImportSeedkeeperInsertView` / `ToolsKeycardImportSeedkeeperReinsertView` — that redirect (and its "Card removed" toast) is **suppressed**, so the screen stays put for the swap instead of dead-ending the second backup/import. Exemption is a declarative classname allowlist (`Controller._CARD_VIEW_REDIRECT_EXEMPT_CLASSNAMES`, checked via `_top_is_card_swap_view()` in `_on_card_removed_redirect` and `dispatch_card_removed_event`); the secret **wipe still fires** (it never touches the pending seed). Scoped to exactly those blocking screens — once the user presses Continue and the flow advances to a card-*present* step (`…SelectView` / `…PushView` / `…SaveRunView`), full redirect protection resumes. Cover: `tests/test_cards_menu_view.py::TestCardRemovedRedirect`.

### No-card UX in scan/sign flows

When a Keycard sign/export flow (ETH finalize, BTC export-xpub / PSBT review / PSBT finalize / message finalize) hits `NoCardError`/`NoReaderError`, it does **not** show the heavy `ErrorScreen`. Instead `_no_card_toast_or_error()` (in `keycard_views.py`) flashes the same subtle `InfoToast("Insert a card first")` the Cards menu uses and **stays** — re-entering the holding view (or `BackStackView`) so the user can insert a card and retry **without re-scanning**. Critically, the no-card branch must **not** null the scanned state (`eth_sign_request` / `psbt` / `btc_parsed_psbt` / the message view-arg) — that is what makes retry-without-rescan work. Any *other* exception still falls through to `classify_card_error` + `_error_destination(..., return_to_main=True)` as before. `KeycardCardChangedError` (a *swapped* card) now follows the **same retry-without-rescan principle**: the primary sign/export finalize handlers (ETH verify/finalize, BTC export-xpub / PSBT review+finalize / message finalize) keep the scanned state and route via `_pair_then_resume(...)` → `ToolsKeycardPairView(next_destination=…)`, which on a successful pair **resumes the originating step** (both hops carry `skip_current_view`, so the back stack ends up as if the pairing detour never happened). They no longer null `eth_sign_request` / `psbt` / `btc_parsed_psbt`.

The **key-push flows** (import / generate / SeedKeeper-import) avoid a related trap on the **first session-op of a boot**, when no pairing is cached yet. Previously the final `LOAD_KEY` push *was* that first session-op: it raised `KeycardCardChangedError`, and the push handled it with a bare `return Destination(ToolsKeycardPairView)` (no resume), which silently paired the card (v3.2 ephemeral PSK — no screen, no toast) and then returned `BackStackView`, **re-running the view from the top and discarding the just-typed seed** while the card stayed empty (the recurring "import doesn't save on the first try" bug; the *second* full attempt worked only because the first one cached the pairing). Now the **import** and **generate** entry views (`ToolsKeycardImportSeedView`, `ToolsKeycardGenerateKeyView`) call `_ensure_card_paired(self)` **before any seed is collected** — a SELECT-only probe (`identify_inserted_card`) that, when the inserted card has no cached ephemeral/persistent pairing, routes through `_pair_then_resume(Destination(type(view)))` to pair first and re-enter the same view (now paired), so the user types the seed **once** and the push succeeds. It is advisory: a probe failure returns `None` and the push surfaces any real error downstream. The `ToolsKeycardGenerateSeedLoadView` / `ToolsKeycardImportSeedkeeperPushView` push steps additionally handle a *mid-flow* `KeycardCardChangedError` via `_pair_then_resume(Destination(<that push view>))` **without** wiping `controller.pending_keycard_*` (the resumed view re-reads the mnemonic from the controller). The import view's own push loop keeps the bare `Destination(ToolsKeycardPairView)` only as a rare-swap backstop (its seed is method-local, so a resume there would re-type). Cover: `tests/test_keycard_import_pairing.py` (+ the happy-path assertion the retry suite lacked).
...
- **PIN cached per-instance, but droppable on demand.** The verified PIN is held in `Controller.keycard_pins` — a dict keyed by `instance_uid`, values are `bytearray` (`controller.py`, helpers `get_pin_for` / `set_pin_for` / `forget_pin_for` / `forget_all_pins`). After the first VERIFY_PIN, subsequent operations on the same instance re-use the cached value **without re-prompting** (`open_unlocked_session`, `ui_helpers.py`). The cache is dropped on **all** of:
  ...
  - the PC/SC **`removed`** event (`card_monitor.py` → `wipe_card_session_secrets()`). This wipe is **immediate and unconditional**, and is **separate** from the debounced *redirect-to-Home* (`dispatch_card_removed_event` → `_on_card_removed_redirect`): the wipe always fires, but the redirect (and its "Card removed" toast) is **suppressed on the deliberate card-swap prompts** so an instructed mid-flow swap isn't dead-ended (see "Swap-screen redirect exemption" under SeedKeeper backup);
...
**Ordering / id / default (so the UI is not surprising).** The active instance is **not** persisted: it resets to the Status published AID `…0101` (`commands.APPLET_AID`) on **every** boot. `Inst N` is the AID's **trailing byte** — a physical applet *slot*, reused after a delete (`_next_free_instance_aid` fills the smallest free slot), **not** a user identity. `Switch instance` list order is whatever `GET STATUS` reports. `Create instance` **auto-activates** the new instance (sets `active_keycard_aid` to the freshly-installed AID) **and then chains straight into the Init wizard** (`ToolsKeycardInstancesCreateView` returns `Destination(ToolsKeycardInitView, skip_current_view=True)`), so after the "Created / Run Init next" confirm the user lands directly on the PIN/PUK setup for the new instance instead of being stranded in the deep Instances submenu (backing out of Init's first PIN prompt falls to the Instances menu). The auto-activate is what makes the chained Init target the right instance — without it, `select_with_autodetect` would first-try the previous, already-initialised instance and bail with "Already initialised". Cover: `tests/test_keycard_views.py::...test_create_auto_switches_active_instance`. `Rename instance` renames the **active** instance — switch to the instance you mean before renaming it (its current name now shows in the `This instance · {name}` title to make the target explicit).
...
**Memory-aware instance limit (how many instances fit).** `MAX_KEYCARD_INSTANCES` (now **16**, raised from 4) is a *firmware* ceiling — bounds the slot byte and the management-flow probe range — **not** a hardware one. Whether another instance actually fits is a free-space estimate, since the card reports only **free** memory (GET DATA 0xFF21 via `query_card_memory`: free **NV**/EEPROM `tag 0x82`, free **volatile**/RAM `tag 0x83`), never total or per-instance usage. **EEPROM is NOT the binding resource — transient RAM is** (CONFIRMED on-device): a field card refused a 5th instance with SW=0x6F00; an on-device GET DATA 0xFF21 read showed raw `8102 0007 / 8204 0000b604 / 8304 0000012f` → free NV=46596 B (~45.5 KB, plenty) but free **volatile=303 B** (essentially exhausted; 7 applets on the card). The 5th install still failed at 303 B free → one instance's transient cost is **> 303 B**. So the estimate now gates on **both** memories *and* learns from real install failures:
  - **Per-instance estimates** (`views.view.keycard_instance_nv_estimate` / `keycard_instance_volatile_estimate`): fixed conservative constants `KEYCARD_INSTANCE_NV_ESTIMATE_BYTES` (**3 KB**) and `KEYCARD_INSTANCE_VOLATILE_ESTIMATE_BYTES` (**0.5 KB**) — **hardware-calibratable** like `CAP_NV_FOOTPRINT_RATIO` (measure free NV/RAM before/after a real create+init; for RAM, see `gp --info` tag 0x83) by editing the constant. Both helpers take a `controller` arg that is now accepted-and-ignored (kept for call-site uniformity). An *instance* costs ~1–2 KB EEPROM (INSTALL shell + INIT/key allocation), **not** the ~30 KB applet *package* (loaded once, shared via `INSTALL [for install]`).
  - **Monotonic capacity cap (replaces the old post-install self-calibration).** The free-memory estimate is *noisy* — transient-RAM readings fluctuate between installs, so the raw `≈N more fit` could jump *upward* as instances were added (the reported **3→6→2** bug). Fix: `views.view.apply_capacity_cap(controller, current_count, raw_remaining, *, update)` anchors on the **predicted total capacity** (`current_count + remaining`) and keeps only its running minimum on `Controller.keycard_capacity_estimate` — the first estimate fixes the predicted maximum; reality (a refused install, freed space) can only lower it (so a max-4 card reads **3 → 2 → 1 → "Card appears full"**). The create flow calls it with `update=True` (it holds the authoritative GET-STATUS count, so it writes the ratchet); the Storage view with `update=False` (its count may be a fallback, so it only *reads* the cap). Card-specific → reset to `None` on swap (`wipe_card_session_secrets`) **and on Delete** (a freed slot lets the estimate recover). The old `_record_instance_nv_measurement` / `keycard_measured_instance_nv` / `keycard_measured_instance_volatile` self-calibration was **removed**: RAM can't be measured post-install (it's reclaimed once the INSTALL buffers free) and the measurement was the actual source of the upward jump — the durable RAM signal is the **install-failure clamp** below.
  - **Install-failure clamp (ground truth, the most reliable signal):** when INSTALL fails with a memory-class SW — `is_install_memory_failure(exc)` in `ui_helpers` matches **0x6A84** (Not enough memory) and, pragmatically in the install context, **0x6F00** (no precise diagnosis), in both the `GpProtocolError` (lowercase SW in the message, via `format_sw_error`) and `APDUError` (`.sw`) shapes — the create flow sets `Controller.keycard_install_full = True`. That forces every subsequent estimate to **0** (create-confirm + Storage short-circuit before `estimate_remaining_instances`). Card-specific → reset on swap **and on Delete** (a freed slot may make room again). `classify_card_error` also maps the unambiguous **0x6A84** to "Card full" for every flow (0x6F00 stays scoped to the create flow — it's too generic to map globally).
  - **`estimate_remaining_instances(free_nv, current_count, per_instance, ceiling, free_volatile=None, per_instance_volatile=None)`** = `min` of slot headroom (`ceiling − current_count`), EEPROM headroom (`(free_nv − LOW_SPACE_MARGIN_BYTES) // per_instance`), and — **only when the card reports a non-zero `free_volatile`** — RAM headroom (`(free_volatile − LOW_SPACE_VOLATILE_MARGIN_BYTES) // per_instance_volatile`). **`free_volatile` 0 or `None` drops the RAM term** (tag 0x83 absent ≠ "no RAM") → falls back to EEPROM+slot; the RAM term only ever *tightens*. `free_nv None` drops the EEPROM term. Both call sites then wrap this raw value in `apply_capacity_cap` (above), so the displayed `≈N` only ever ratchets **down**.
  - **UX:** create-confirm shows `≈{N} more fit`, or **"Card appears full"** when `remaining <= 0` (never "≈0 more fit"); the soft `DireWarningScreen` reads "Card appears full / Create may fail" once the card has actually refused (`keycard_install_full`), else "Card may be too full / for another instance" — both keep the **"Create anyway"** override (never a hard block; only the 16 ceiling is). The Storage view (`ToolsKeycardStorageView` → `KeycardStorageScreen`) shows free EEPROM **and free RAM** (folded into the detail TextArea as a 3rd line — tight 200 px budget) plus `≈{N} more instances fit`. Cover: `tests/test_keycard_storage.py::TestInstanceNvEstimate` / `TestInstanceVolatileEstimate` (return the conservative constant) / `TestEstimateRemainingInstances` (RAM-binding, free_volatile=0/None fallback) / `TestApplyCapacityCap` (monotonic ratchet, read-only Storage clamp), `tests/test_keycard_views.py::TestCreateInstanceMemoryGate` (clamp on 0x6A84/0x6F00, "Card appears full", cap anchors the predicted total + clamps the confirm so `≈N` never jumps up, `test_delete_resets_capacity_clamp_and_cap`) / `TestIsInstallMemoryFailure`, `tests/test_controller.py` (wipe resets cap + clamp).
