...
**Memory-aware instance limit (how many instances fit).** `MAX_KEYCARD_INSTANCES` (now **16**, raised from 4) is a *firmware* ceiling — bounds the slot byte and the management-flow probe range — **not** a hardware one. Whether another instance actually fits is a free-space estimate, since the card reports only **free** memory (GET DATA 0xFF21 via `query_card_memory`: free **NV**/EEPROM `tag 0x82`, free **volatile**/RAM `tag 0x83`), never total or per-instance usage. **EEPROM is NOT the binding resource — transient RAM is** (CONFIRMED on-device): a field card refused a 5th instance with SW=0x6F00; an on-device GET DATA 0xFF21 read showed raw `8102 0007 / 8204 0000b604 / 8304 0000012f` → free NV=46596 B (~45.5 KB, plenty) but free **volatile=303 B** (essentially exhausted; 7 applets on the card). The 5th install still failed at 303 B free → one instance's transient cost is **> 303 B**. So the estimate now gates on **both** memories *and* learns from real install failures:
  - **Per-instance estimates** (`views.view.keycard_instance_nv_estimate` / `keycard_instance_volatile_estimate`): `KEYCARD_INSTANCE_NV_ESTIMATE_BYTES` (**3 KB**) and `KEYCARD_INSTANCE_VOLATILE_ESTIMATE_BYTES` (**0.5 KB**, PLACEHOLDER) — both conservative, **hardware-calibratable** like `CAP_NV_FOOTPRINT_RATIO` (measure free NV/RAM before/after a real create+init; for RAM, see `gp --info` tag 0x83) — **unless** a card-specific measurement exists. An *instance* costs ~1–2 KB EEPROM (INSTALL shell + INIT/key allocation), **not** the ~30 KB applet *package* (loaded once, shared via `INSTALL [for install]`).
  - **Self-calibration (now implemented):** after a successful INSTALL, `ToolsKeycardInstancesCreateView` disconnects the SCP02 channel and re-reads free NV+RAM on a **FRESH clear channel** (`query_card_memory`, no second `open()`), then `_record_instance_nv_measurement` stashes the sanity-bounded deltas in `Controller.keycard_measured_instance_nv` (256..16384 B) / `keycard_measured_instance_volatile` (32..4096 B). **Must** be a fresh channel — an un-MAC'd GET DATA on the open post-install channel breaks it (same constraint as the pre-open read). RAM is frequently *reclaimed* post-install (transient APDU buffers freed) so the volatile delta is often rejected → the durable RAM signal is the **install-failure clamp** below, not this measurement. Both fields are card-specific → cleared on swap (`wipe_card_session_secrets`).
  - **Install-failure clamp (ground truth, the most reliable signal):** when INSTALL fails with a memory-class SW — `is_install_memory_failure(exc)` in `ui_helpers` matches **0x6A84** (Not enough memory) and, pragmatically in the install context, **0x6F00** (no precise diagnosis), in both the `GpProtocolError` (lowercase SW in the message, via `format_sw_error`) and `APDUError` (`.sw`) shapes — the create flow sets `Controller.keycard_install_full = True`. That forces every subsequent estimate to **0** (create-confirm + Storage short-circuit before `estimate_remaining_instances`). Card-specific → reset on swap. `classify_card_error` also maps the unambiguous **0x6A84** to "Card full" for every flow (0x6F00 stays scoped to the create flow — it's too generic to map globally).
  - **`estimate_remaining_instances(free_nv, current_count, per_instance, ceiling, free_volatile=None, per_instance_volatile=None)`** = `min` of slot headroom (`ceiling − current_count`), EEPROM headroom (`(free_nv − LOW_SPACE_MARGIN_BYTES) // per_instance`), and — **only when the card reports a non-zero `free_volatile`** — RAM headroom (`(free_volatile − LOW_SPACE_VOLATILE_MARGIN_BYTES) // per_instance_volatile`). **`free_volatile` 0 or `None` drops the RAM term** (tag 0x83 absent ≠ "no RAM") → falls back to EEPROM+slot; the RAM term only ever *tightens*. `free_nv None` drops the EEPROM term.
  - **UX:** create-confirm shows `≈{N} more fit`, or **"Card appears full"** when `remaining <= 0` (never "≈0 more fit"); the soft `DireWarningScreen` reads "Card appears full / Create may fail" once the card has actually refused (`keycard_install_full`), else "Card may be too full / for another instance" — both keep the **"Create anyway"** override (never a hard block; only the 16 ceiling is). The Storage view (`ToolsKeycardStorageView` → `KeycardStorageScreen`) shows free EEPROM **and free RAM** (folded into the detail TextArea as a 3rd line — tight 200 px budget) plus `≈{N} more instances fit`. Cover: `tests/test_keycard_storage.py::TestInstanceNvEstimate` / `TestInstanceVolatileEstimate` / `TestEstimateRemainingInstances` (RAM-binding, free_volatile=0/None fallback), `tests/test_keycard_views.py::TestCreateInstanceMemoryGate` (clamp on 0x6A84/0x6F00, "Card appears full", post-install delta measurement) / `TestIsInstallMemoryFailure`, `tests/test_controller.py` (wipe resets calibration + clamp).
