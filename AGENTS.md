...
- **Full release ceremony**: the end-to-end runbook for cutting a paired release (version bump, tags + prereleases, submodule pin, docs/screenshots, CI dispatches for the four build lineages, and local rebuild with byte-identical SHA-256 verification) lives in the OS repo at `tools/release-verify/SKILL.md` (plus the `launch-pi.sh` / `launch-luckfox.sh` / `compare-hashes.py` helpers beside it).
